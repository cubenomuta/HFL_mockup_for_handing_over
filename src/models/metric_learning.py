...
from torchvision.models.resnet import BasicBlock, Bottleneck, ResNet18_Weights, conv1x1
    ...
    def forward(self, w, out_dims):
        ...
        for i in range(out_dims - 1):
            for j in range(i + 1, out_dims):
                loss += torch.pow(torch.max(torch.tensor(0.0), self.nu - 1 + torch.dot(w[i, :], w[j, :])), 2)
        return loss
